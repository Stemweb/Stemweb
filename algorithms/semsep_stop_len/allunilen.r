...
Initialuni <- function (fileread)
{
        ...
        lenmat = matrix(0.05,ncol=AllNodeNumber, nrow=AllNodeNumber)
        colnames(lenmat)=NodeList       
        rownames(lenmat)=NodeList
        ...
        'AllNodeNumber'=AllNodeNumber, 'lenmat'=lenmat,
...
#                         print ('************')
#                         print (lenmat)
#                         print (lenmatposition)
#                         print (PositionDiffTmp-1)
...
{ 
	...
	{	
		...
		if (qscorechange > 0.00001) # if qscore not converged
		{
			...
			lenmat = runres[[run]][["lenmat"]]

			runrestmp[[run]] = uniiteration(MTreeunires=MTreeunires,rhoin=rhoin,deltain=deltain,
			qscorevector=qscorevector,Initialres=Initialres,approximationin=approximation,lenmat=lenmat)
...
findbestrun <- function(iterationrunres, runmax)
{
	...
	bestqscore = bestscoretmp[bestruntmp]
	findbestrunres = list("bestruntmp"=bestruntmp,"bestqscore"=bestqscore)

	invisible (findbestrunres)
}
...
updateres <- function(runmax, bestruntmp, iterationrunres)
{
	...
	for (run in c(1:runmax))
	{
		...
		#### update "bestres"
		iterationrunres[["runres"]][[run]][["MTreeunires"]] = bestrestmp[[bestruntmp]] [["MTreeunires"]] 
		#### update "runres"--"MTreeunires"
