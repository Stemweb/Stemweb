runsemf81<-function(infile, outfolder, runmax, itermaxin, approximationin=0)
{
	...
	MTreef81 <- function(resfolder, iter, AllNodeNumber, LinkMatAllori,LinkMatAll, qscoreold)
	{
		...
		if (qscoreold<qscore)
		{
			...
			system(paste('chmod 777 ',resfolder,'/treebest.txt',sep=''))
	...
	rhoin,deltain,qscorevector,Initialres,MTreef81res)
	{
		...
		{	
			...
			if (stop==1|iternow==itermaxin)
			{
				...
				system(paste('chmod 777 ',resfolderin,'/treelast.txt',sep=''))
...
#	system(paste('chmod 777 ',resfolder,sep=''))
	...
	{
		resfolder2 = paste(outfolder,'/run',run,sep='')
	...
	system(paste('chmod 777 ',outfolder,'/run',runlastbest,'/treelast.txt',sep=''))
	file.copy(from=paste(outfolder,'/run',runlastbest,'/treelast.txt',sep=''), 
	to=paste(outfolder,'/lastbest',sep=''))

	system(paste('chmod 777 ',outfolder,'/run',runlastbest,'/treebest.txt',sep=''))
	file.copy(from=paste(outfolder,'/run',runbest,'/treebest.txt',sep=''), 
	to=paste(outfolder,'/allbest',sep=''))
	cat(paste('lastbest\t',runlastbest,'\t',qscorerun[runlastbest,2],'\n',
	'allbest\t',runbest,'\t',qscorerun[runbest,1],sep=''),
	
	file=paste(outfolder,'/','resultsummary',sep=''))
	system(paste('chmod 777 ',outfolder,'/','resultsummary',sep=''))
	cat (paste('********************************************************\n\n'))
	...
	system(paste('chmod 777 ',outfolder,'/allbest',sep=''))
	nettomatrix(netfile=paste(outfolder,'/allbest',sep=''))
	nettodot(netfile=paste(outfolder,'/allbest',sep=''))

	system(paste('chmod 777 ',outfolder,'/lastbest',sep=''))
	nettomatrix(netfile=paste(outfolder,'/lastbest',sep=''))
	nettodot(netfile=paste(outfolder,'/lastbest',sep=''))
