runsemuni<-function(infile, outfolder, runmax, itermaxin, ProbSamein=0.95, approximationin=0)
{

	# function 1

	Initialuni <- function (ProbSame, fileread)
	{
	...
	PositionProb, delta, TextLen, ProbSame, approximation)
	{
		localtimer=0
	...
	MTreeuni <- function(resfolder, iter, AllNodeNumber, LinkMatAllori,LinkMatAll, qscoreold)
	{
	...
	allstart <- function (ProbSamein, resfolderin, itermaxin,
	filein, approximationin)
	{
		...
		Initialres <- Initialuni(ProbSame = ProbSamein, fileread = filein)
		...
		ProbSame = ProbSamein, approximation = approximationin)
		...
		{	
			...
			PositionDiff=Initialres[['PositionDiff']],
			PositionProb= Initialres[['PositionProb']], delta = deltain, 
			TextLen=Initialres[['TextLen']], ProbSame = ProbSamein, 
	...
	allend <- function (ProbSamein, resfolderin, itermaxin,
	filein, approximationin,
	rhoin,deltain,qscorevector,Initialres,MTreeunires)
	{
		Sys.setlocale(locale="C")
		stop = 0
	...
	for (run in c(1:runmax))
	{
		...
		allstartres[[paste(run)]]<-allstart(ProbSamein, 
		resfolderin = resfolder2, 
		filein = nexfile,
		itermaxin=itermaxin, 
	...
	for (run in c(1:runmax))
	{
		...
		qscoreruntmp<-allend(ProbSamein, 
		resfolderin = resfolder2, 
		filein = nexfile,
		itermaxin=itermaxin, 
		approximationin=approximationin,
