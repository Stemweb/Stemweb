	...
	for nodei in nodeorder:
		if len(treedic[nodei]['parent'])>0:
			for nodej in treedic[nodei]['parent']:
				outstr = outstr + '\t' + nodei + ' -- ' + nodej + '[len = 1];\n'
	outstr = outstr + '}'
	file = open(os.path.join(resultfolder,resultfile) +'.dot','w')
	file.write(outstr)
	file.close()
	os.system('neato -Tpdf -Gstart=rand ' + os.path.join(resultfolder,resultfile) + '.dot > ' + os.path.join(resultfolder,resultfile) +'.pdf')
